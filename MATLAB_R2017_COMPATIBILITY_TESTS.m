...
[sessionInput, sessionOutput, sessionStatus, sessionState] = ...
	Calc_OutputWithState(sampleTime, struct(), 1);
[nextSessionInput, nextSessionOutput, nextSessionStatus, sessionState] = ...
	Calc_OutputWithState(sampleTime, sessionState, 0);
assert(isequal(sessionInput, calculatedInput) && isequal(sessionOutput, calculatedOutput), ...
	'Caller-owned session output should match the compatibility wrapper.');
assert(isscalar(sessionStatus) && isscalar(nextSessionStatus), ...
	'Caller-owned session should return scalar statuses.');
assert(all(isfinite(nextSessionInput)) && all(isfinite(nextSessionOutput)), ...
	'Caller-owned session should remain finite across periods.');
